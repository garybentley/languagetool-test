...
public abstract class AbstractPatternRuleHandler<T extends Rule> extends XMLRuleHandler<T> {
  ...
  protected final List<DisambiguationPatternRule> rulegroupAntiPatterns = new ArrayList<>();
  protected final List<DisambiguationPatternRule> ruleAntiPatterns = new ArrayList<>();
...
// GTODO: Make all events have an associated call that can be overriden?

  public AbstractPatternRuleHandler (RuleFilterCreator ruleFilterCreator) {
  ...
  /**
   * Reset the start position to the specified value.  This method is named this way to indicate that the otherwise managed value is
   * being reset to a specific value because a sub-class or client deliberately wishes to change its value (and knows what they are doing).
   *
   * @param v The value to reset to.
   */
  public void resetStartPositionTo(int v) {
      startPos = v;
  }
  ...
  /**
   * Reset the end position to the specified value.  This method is named this way to indicate that the otherwise managed value is
   * being reset to a specific value because a sub-class or client deliberately wishes to change its value (and knows what they are doing).
   *
   * @param v The value to reset to.
   */
  public void resetEndPositionTo(int v) {
      endPos = v;
  }
  ...
  /**
   * Called from the RULES case of startElement.
   */
  public abstract void handleStartElementRulesCase(Attributes attrs);
  ...
  public void startElement(String namespaceURI, String lName,
    ...
    switch (qName) {
      case "category":
        ...
        break;
      case RULES:
        handleStartElementRulesCase(attrs);
        /*
  ...
  /**
   * Create the relevant rule(s), called from the RULE case in endElement.
   */
  public abstract void handleEndElementRuleCase() throws Exception;

  /**
   * Create the example(s), called from the EXAMPLE case in endElement.
   */
  public abstract void handleEndElementExampleCase();
  ...
  public void endElement(String namespaceURI, String sName,
    ...
    switch (qName) {
      ...
      case RULE:
        try{
            handleEndElementRuleCase();
            inRule = false;
        } catch(Exception e) {
            throw new SAXException("Unable to handle rule case.", e);
        }
        break;
        /*
   ...
   * @param classLoader The class loader to use to load classes needed for the rule.
   */
   /*
   GTODO: Clean up
